...
runset = function(set, pos, finished)
  ...
  if finished == nil then finished = function() end end
  local current = set[pos]
  if not (current == '') then
    local child = exec(current, function(_, _, ...) return print(...) end)
    ...
    if #set > pos then
      if #set > pos then return runset(set, pos + 1) end
    else
      return finished()
    end
  end
...
if existsSync(file) then
  ...
  if not (code:sub(0, 6) == 'return') then code = 'return ' .. code end
  local env = setmetatable({}, {__index = _G})
  local fn, syntax = load(code, 'make:' .. tostring(file), 't', env)
  if not (fn) then return error(syntax) end
