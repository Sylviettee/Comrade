...
local colors = {[1] = ansicolors.noReset('%{cyan}'), [2] = ansicolors.noReset('%{white}')}
...
local function escape_chars(c) return '\\' .. string.byte(c) end
...
local function show_primitive(t, color, seen)
  ...
  if strlen(s) > 80 then
    ...
    for i = 1, strlen(s), 80 do output_line(out, strsub(s, i, i + 79)) end
  else
    output_line(out, s)
  end
...
show_as_columns = function(t, bgcolor, seen, column_order, skip_header)
  ...
  if column_order then
    ...
    column_set = {}
    for _, cname in ipairs(column_names) do column_set[cname] = true end
  end
  ...
  local function output_cell(line, cname, text, color)
    ...
    if color then table.insert(line, bgcolor) end
  ...
  for k, v in pairs(t) do
    if type(k) ~= 'number' then
      ...
      for _, line in ipairs(out2) do output_line(out, line) end
      break
    end
  end
