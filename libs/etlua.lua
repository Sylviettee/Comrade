...
setfenv = setfenv or function(fn, env)
  ...
  while true do
    ...
    if not name or name == '_ENV' then break end
    i = i + 1
  end
  if name then debug.upvaluejoin(fn, i, (function() return env end), 1) end
  return fn
end
...
do
  ...
  local _base_0 = {
    html_escape = true,
    render = function(self) return table.concat(self.buffer) end,
    push = function(self, str, ...)
      local i = self.i + 1
      self.buffer[i] = str
      self.i = i
      if ... then return self:push(...) end
    end,
    header = function(self) return self:push('local _tostring, _escape, _b, _b_i = ...\n') end,
    footer = function(self) return self:push('return _b') end,
    increment = function(self) return self:push('_b_i = _b_i + 1\n') end,
    mark = function(self, pos) return self:push('--[[', tostring(pos), ']] ') end,
    assign = function(self, ...)
      self:push('_b[_b_i] = ', ...)
      if ... then return self:push('\n') end
    end,
...
do
  ...
  local _base_0 = {
    ...
    next_tag = function(self)
      ...
      if not (start == self.pos) then self:push_raw(self.pos, start - 1) end
      self.pos = stop + 1
      local modifier
    ...
    push_raw = function(self, start, stop) return insert(self.chunks, self.str:sub(start, stop)) end,
    ...
    run = function(self, fn, env, buffer, i, ...)
      ...
      local combined_env = setmetatable({}, {
        __index = function(self, name)
          local val = env[name]
          if val == nil then val = _G[name] end
          return val
        end
      })
      if not (buffer) then
    ...
    chunks_to_lua = function(self, compiler_cls)
      if compiler_cls == nil then compiler_cls = Compiler end
      return compiler_cls():compile_chunks(self.chunks)
    end
  }
  _base_0.__index = _base_0
  _class_0 = setmetatable({__init = function() end, __base = _base_0, __name = 'Parser'}, {
    __index = _base_0,
    __call = function(cls, ...)
      local _self_0 = setmetatable({}, _base_0)
      cls.__init(_self_0, ...)
...
return {compile = compile, render = render, Parser = Parser, Compiler = Compiler, _version = VERSION}
