  ...
  if not (self._testbot or self._disableDefaultCH) then
    return self:on('messageCreate', function(msg)
      ...
      if not (perms:has(enums.permission.sendMessages)) then
        return self:debug("Harmonia : No send messages")
      end
      ...
      if found then
        self:debug("Harmonia : Ran " .. tostring(command))
        local succ, err = pcall(function()
          return found:run(msg, args, self)
        end)
        if not (succ) then
          self:debug("Harmonia : Error " .. tostring(err))
...
helper.addCommand = function(self, command)
  self:debug("Harmonia: New command " .. tostring(command.name))
...
helper.addEvent = function(self, event)
  self:debug("Harmonia: New listener " .. tostring(event.name))
  event:use(self)
