...
local options = {
  ...
  'autoReconnect', 'compress', 'bitrate', 'logFile', 'logLevel', 'gatewayFile', 'dateTime', 'syncGuilds'
}
helper.__init = function(self, token, config)
  if config == nil then config = {} end
  local clientConfig = {}
  for i, v in pairs(config) do if table.search(options, i) then clientConfig[i] = v end end
  ...
  self._ready = false
  if self._botid then table.insert(self._owners, self._botid) end
  self._start = os.time()
...
helper.resetErrors = function(self) self._errors = {} end
helper.updateOwners = function(self, owners) self._owners = owners end
...
helper.removeCommand = function(self, name, check)
  if check == nil then check = function() return false end end
  return self._commands:forEach(function(command, pos)
    if command.name == name or check(command, name) then return self._commands:pop(pos) end
  end)
end
...
helper.removePlugin = function(self, name)
  self:removeCommand('', function(com) return com.parent == name end)
  return self:removeEvent('', function(event) return event.parent == name end)
end
get.start = function(self) return self._start end
get.commands = function(self) return self._commands end
