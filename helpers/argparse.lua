...
split = function(str)
  local args = {}
  for i in gmatch(str, [[(?|"(.+?)"|'(.+?)'|(\S+))]]) do table.insert(args, i) end
  return args
end
local isSnowflake
isSnowflake = function(id) return type(id) == 'string' and #id >= 17 and #id <= 64 and not id:match('%D') end
do
  local _class_0
  local _base_0 = {
    execute = function(self, msg, args)
      ...
      for _, value in pairs(self.args) do
        ...
        table.insert(parsed, {default = default, id = value.id, type = self.__class:typeParser(value.type)})
      end
  ...
  _class_0 = setmetatable({
    __init = function(self, tbl, command)
      ...
      if command then
        local required = self.args:filter(function(arg) return arg.default == nil end)
        local optional = self.args:filter(function(arg) return arg.default ~= nil end)
        command.usage = tostring(command.name) .. tostring((#required > 0 and '<' .. table.concat(
                                                               (function()
              local _accum_0 = {}
              local _len_0 = 1
              ...
              return _accum_0
            end)(), ', ') .. '>') or '') .. ' ' .. tostring((#optional > 0 and '[' .. table.concat(
                                                                (function()
              local _accum_0 = {}
              local _len_0 = 1
  ...
  self.types = {
    ...
    string = function(val) return (tonumber(val) and nil) or val end,
    ...
    guild = function(val, msg)
      ...
      elseif #name >= 2 and #name <= 100 then
        return msg.client.guilds:find(function(guild) return guild.name:lower():find(name:lower(), 1, true) end)
      else
        return nil
      end
    end,
    ...
    role = function(val, msg)
      ...
      if name and not id and msg.guild then
        return msg.guild.roles:find(function(r) return r.name:lower():find(name:lower(), 1, true) end)
      end
    ...
    textchannel = function(val, msg)
      ...
      elseif name then
        channel = guild.textChannels:find(function(chan) return chan.name:lower() == name:lower() end)
      end
      local types = {'TextChannel', 'GuildTextChannel'}
