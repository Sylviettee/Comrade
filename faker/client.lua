...
faker.__init = function(self, token, config)
  ...
  return self:on('ready', function()
    if self._defaultChannel then
      self._defaultChannel = self:getChannel(self._defaultChannel)
    end
  end)
end
...
faker.case = function(self, test, fn)
  return self._testStack:push({
    test = test,
    fn = fn
  })
end
faker.execute = function(self, content, channel)
  ...
  local didReply, reply = self:waitFor('messageCreate', self._waitTime, function(msg)
    return msg.author.id == self._mainbot.user.id
  end)
  return didReply and reply or nil
end
faker.load = function(self, dir)
  local tests = Array(unpack(readdirSync(dir))):filter(function(file)
    return file:match('.+%.spec%.lua$')
  end)
  return tests:forEach(function(file)
    ...
    sandbox.describe = function(...)
      return self._testStack:push(Describe(...))
    end
    local fn, syntax = load(test, 'Test', 't', sandbox)
    if not (fn) then
      return logger.test(file, syntax)
    end
    local succ, err = pcall(fn)
    if not (succ) then
      return logger.test(file, err)
    end
  end)
end
...
get.errored = function(self)
  return #self._mainbot.errors > 0 or #self._errors > 0
end
